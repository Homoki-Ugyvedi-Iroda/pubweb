...
[^3]: DeepSeek model pages on Hugging Face for the open-weight models tested here:
    ...
    The non-reasoning baseline "DeepSeek Chat" (`deepseek-chat`), which is actually just a non-thinking mode of the same DeepSeek-v4-Flash; see <https://huggingface.co/deepseek-ai> for the full catalogue.
This experiment cost me **$65** direct API spend across all 19 runs, drafting, verification, and analysis, plus the annual Claude Pro subscription I had. I was using Claude Code (Anthropic's agentic CLI) to write the scripts, most of the analysis etc. (but not this blogpost - just footnotes, spelling errors etc.). If you want to understand the costs of the LLMs, you have to understand that it breaks input and output text into chunks called tokens (sub-word units, roughly 4 characters for an English word, less for Hungarian etc). Tokens are what providers measure and bill, both for what you send in and what the model emits.

## 2\. An SQE1 leaderboard 

| Rank | Model (run) | Family | Success | Cost (list) | sec/Q |
|---:|---|---|---:|---:|---:|
| 1 | DeepSeek Flash (4K) | DeepSeek | 87.2 % | ~$0.05 | 10 |
| 2 | DeepSeek Pro 16K | DeepSeek | 85.3 % | ~$0.10 | 26 |
| 3 | Claude Opus 4.8 | Anthropic | 85.0 % | ~$3.50 | not measured |
| 4 | DeepSeek Pro | DeepSeek | 84.5 % | ~$0.08 | 26 |
| 5 | Claude Opus 4.5 | Anthropic | 83.4 % | ~$3.00 | not measured |
| 6 | Claude Opus 4.0 | Anthropic | 81.6 % | ~$3.00 | not measured |
| 7 | Claude Sonnet 4.6 (mean of 3 runs) | Anthropic | 79.1 % | ~$0.64 | not measured |
| 8 | Claude Haiku 4.5 + 2K thinking | Anthropic | 75.3 % | $1.62 | 8 |
| 9 | Claude Haiku 4.5 + 4K thinking | Anthropic | 75.0 % | $2.37 | 11 |
| **10** | **Human (me)** | Homoki | 67.1 % | ~$6.53 | 102 |
| 11 | DeepSeek Chat (mean of 2 runs) | DeepSeek | 66.1 % | ~$0.04 | 1 |
| - | _Expected minimum pass (~60 %)_ | - | _~60 %_ | - | - |
| 12 | Claude Haiku 4.5 (mean of 3 runs) | Anthropic | 48.2 % | ~$0.18 | not measured |
...
|---|---:|---:|---:|
| DeepSeek Chat | ~0.8 s | ~$0.04 | 66 % |
| Claude Haiku 4.5 + 2K thinking | ~8 s | $1.62 | 75 % |
| DeepSeek Flash | ~10 s | ~$0.05 | 87 % |
| DeepSeek Pro | ~26 s | ~$0.08 | 85 % |
| **Human (me)** | ~102 s | ~$6.53 | 67 % |

The DeepSeek "thinking" models (Flash and Pro) are 10–30× slower than the DeepSeek non-thinking model; Pro spends 2.5× more time than Flash for less accuracy. As a reference point, I spend roughly 4× longer per question than the slowest reasoning model - and over a hundred times longer than the DeepSeek Chat model, having the same performance when being 100 times slower and 163 times more expensive.
...
Three models were re-run on the identical 580 questions, with no setting changed between runs:

| Model | Runs | Per-run success (%) | Mean | Max gap | Std. deviation | Similarity (Jac.) |
|---|---:|---|---:|---:|---:|---:|
| Haiku 4.5 | 3 | 49.1 / 47.9 / 47.4 | 48.2 | 1.7 % point | 0.7 % point | 0.75 |
| Sonnet 4.6 | 3 | 79.5 / 79.5 / 78.4 | 79.1 | 1.0 % point | 0.5 % point | 0.92 |
| DeepSeek Chat | 2 | 65.5 / 66.7 | 66.1 | 1.2 % point | 0.6 % point | 0.73 |
...
Two things to see:
  * **Max gap is similar across all three models** (1.0 – 1.7 % points). If you re-run the same benchmark you should expect about a percentage-point difference. That also means any leaderboard gap under ~2 % points is like noise.
  * **Similarity is different.** The similarity of mistakes in Sonnet at 0.92 is very high - when Sonnet was asked the same question on different days it almost always made the same decision; the 1.0-% point difference comes from a handful of borderline questions. While Haiku and DeepSeek Chat reached roughly the same total success each time, but a different slice of questions were wrong. A single run of Haiku may get a different 5 % of questions wrong than the next run.

 A larger or better-calibrated model can be predictably wrong on the same questions, and a smaller one can be wrong on different questions each time. For lawyers, this second pattern is terrible, because this produces inconsistent results.
 So, for us, it's very important to see not just X % on a specific benchmark, but also the variance.

## 6\. Lesson 4 - Price does not always mean better performance, and worse does not always mean cheaper

  * DeepSeek Flash (\~$0.05/run): 87.2 %
  * Opus 4.8 (\~$3.50/run, \~70× the price): 85.0 %

Implication: it is always dependant on the specific task when a tool's marketing implies "we use the most expensive model so you get the best results", that's not how this works for any single task
| Model | Cost (580 Q) | Correct / 580 | Cost per correct answer |
|---|---:|---:|---:|
| DeepSeek Chat (mean of 2) | ~$0.04 | 384 | $0.0001 |
| DeepSeek Flash | ~$0.05 | 506 | $0.0001 |
| DeepSeek Pro | ~$0.08 | 490 | $0.0002 |
...
| Claude Opus 4.8 | ~$3.50 | 493 | $0.0071 |

Opus 4.8 is the third most accurate model in this benchmark. Sorted by cost per correct answer it is last. The cheapest model that produces a single correct answer here is the open-weight, non-reasoning DeepSeek Chat - about 70× cheaper per correct answer than Opus, despite Opus getting 109 more questions right.

## 7\. Lesson 5 - Different model families fail on different questions

Anthropic and DeepSeek miss largely different, non-overlapping questions.

7 questions out of 580 were wrong for **every** model in **every** run. Those represent questions where models are confidently and uniformly wrong.
...
| **Anthropic ↔ DeepSeek (cross-family pair)** | **0.15 – 0.30** |

Two runs of the same model overlap on their wrong questions 70–90 % of the time. Two models from different families overlap on their wrong questions less than 30 % of the time.

**Table - consensus failures (questions wrong across many models):**

| Wrong in N of the 11 full runs | Same Question_ID |
|---:|---:|
| Every single run (11 / 11) | 7 |
| 9 or 10 of 11 runs | 29 |
| Wrong for me as well, out of the 7 consensus failures | 3 |

Seven questions out of 580 were wrong for every model in every run. I also got wrong three of those seven. This means that a human and lots of different LLMs all choose the same wrong answer.

## 8\. Code and further details

Tools and results are posted at \[https://github.com/Homoki-Ugyvedi-Iroda/\[…]](https://github.com/Homoki-Ugyvedi-Iroda/%5B…%5D)
Includes: 220 SRA sample questions, full taxonomy, the Flask UI, scripts to bring your own wrong-QIDs CSV, the analysis tools
Excludes: commercial materials I have relied on for building my internal test bank.
License: MIT for the code I wrote; separate NOTICE for the SRA samples (presumed © SRA / Kaplan, reproduced from public source - just transformed from PDF to text)
